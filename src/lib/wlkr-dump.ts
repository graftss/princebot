...
class ScaleViolation {
    ...
    toCsvRow(): string {
        return [this.mission, this.rule, this.monoNameIndex, ...this.scale.toFields()].join(',');
    }
}
...
class WlkrDump implements WlkrObjIndex {
            ...
            forEach(ruleIndex, (rows, rule) => {
                ...
                rows.forEach(row => {
                    ...
                    if (levelScales[monoNameIndex] === undefined) {
                        ...
                        const scaleMatch = levelScales[monoNameIndex].find(scale => scale.equals(rowScale));
                ...
                forEach(levelScales, (scales, monoNameIndex) => {
                    if (scales.length > 1) {
                        scales.forEach(scale => {
                            violations.push(new ScaleViolation(mission, rule, monoNameIndex, scale));
                        });
                    }
                });
            });
    ...
    static parseDump(): WlkrDumpRow[] {
        const intFields = ['ctrlIndex', 'monoNameIndex', 'stage', 'submap', 'area', 'mission', 'rule', 'subrule', 'subLocID'];
